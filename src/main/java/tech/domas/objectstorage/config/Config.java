package tech.domas.objectstorage.config;

public class Config {
    public static final String NANO_HTTPD_SERVER_PORT = "nanohttpd.server.port";
    public static final String NANO_HTTPD_SERVER_DAEMON = "nanohttpd.server.daemon";
    public static final String NANO_HTTPD_SERVER_SOCKET_READ_TIMEOUT = "nanohttpd.server.socketreadtimeout";
    public static final String STORAGE_PATH = "storage.path";
    public static final String SQLITE_FILE_NAME = "sqlite.file.name";
    public static final String SQLITE_USER = "sqlite.user";
    public static final String SQLITE_PASS = "sqlite.pass";
}
